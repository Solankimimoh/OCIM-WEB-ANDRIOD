...
public class BookAppoinmentActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initView() {
        ...
        purposeEd = findViewById(R.id.activity_book_appoinment_purpose);
        dateTv = findViewById(R.id.activity_book_appoinment_date);
        timeTv = findViewById(R.id.activity_book_appoinment_time);
        remarkEd = findViewById(R.id.activity_book_appoinment_remarks);
        bookBtn = findViewById(R.id.activity_book_appoinment_book_btn);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.activity_book_appoinment_book_btn:
                bookAppoinment();
                break;

            case R.id.activity_book_appoinment_date:
                openDatePicker();
                break;

            case R.id.activity_book_appoinment_time:
                openTimePicker();
                break;
    ...
    private void bookAppoinment() {
        ...
        final String doctorUUID = intent.getStringExtra(AppConfig.KEY_DOCTOR_UID);
        ...
        if (purpose.isEmpty() || date.isEmpty()
            ...
            if (!uid.isEmpty()) {
                ...
                databaseReference.child(AppConfig.FIREBASE_DB_APPOINMENT)
                        .child(doctorUUID)
                        .child(uid)
                        .setValue(new AppoinmentModel(purpose, date, time, remarks, false,
                                new PrescriptionModel(),
                                new PatientReviewModel()), new DatabaseReference.CompletionListener() {
                            @Override
                            public void onComplete(@Nullable DatabaseError databaseError, @NonNull DatabaseReference databaseReference) {
                                if (databaseError != null) {
                                    Toast.makeText(BookAppoinmentActivity.this, "Database Error :" + databaseError.getMessage(), Toast.LENGTH_SHORT).show();
                                } else {

                                    Toast.makeText(BookAppoinmentActivity.this, "Appoinment Success ", Toast.LENGTH_SHORT).show();
                                    finish();
                                }
                            }
                        });
    ...
    private void openTimePicker() {
        ...
        final TimePickerDialog timePickerDialog = new TimePickerDialog(BookAppoinmentActivity.this, new TimePickerDialog.OnTimeSetListener() {
            @Override
