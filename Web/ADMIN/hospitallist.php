                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-header">
                            <strong class="card-title">Hospital</strong>
                        </div>
                        <div class="card-body">
        ...
        $(document).ready(function() {
            ...
            ref.once('value', function(snapshot) {
                snapshot.forEach(function(childSnapshot) {
                    ...
                    if (childData.verify == false) {

                        var length = mytable.rows.length;
                        mytable.insertRow(length).outerHTML = `<td><img src='${childData.photoUrl}' width='100px'/></td><td>${childData.name}</td>
<td>${childData.email}</td>
<td>${childData.licence}</td>
<td>${childData.category}</td>
<td>${childData.caretype}</td>
<td>${childData.medicine}</td>
<td>${childData.mobile}</td>
<td>${childData.password}</td>
<td>${childData.services}</td>
<td>${childData.website}</td>
<td><button onclick='test("${childSnapshot.key}")' class='btn btn-primary'>APPROVE</button></td>`;
                    }
        ...
        function test(e) {
            ...
            var ref = firebase.database().ref().child("hospital").child(e).child("verify").set(true);
           alert("approved Successfully");
            
                        window.location.href="hospitallist.php";
